...
class conference extends BaseHttp
{
	 ...
	 * @return bool
	 */
	public static function addArticle($title,$subtitle,$source,$author,$gameId,$titlePic,$albumId,$videoId,$catalog,$summary,$content,$tags,$args=array())
	{
		...
		if($result!==false && $result['errorCode']==0){
			$articleId = $result['result'];
			WebToMobileService::syncNewsToQueue($articleId,$title,$subtitle,$source,$author,$gameId,$titlePic,$albumId,$videoId,$catalog,$summary,$content,$tags,$args);
			return true;
		}
	 ...
	 * @return bool
	 */
	public static function updateArticle($id,$title,$subtitle,$source,$author,$gameId,$titlePic,$albumId,$videoId,$catalog,$summary,$content,$tags,$args=array())
	{
		...
		$result = self::http($url,$params,'POST');
		if($result!==false && $result['errorCode']==0){
			WebToMobileService::syncNewsToQueue($id,$title,$subtitle,$source,$author,$gameId,$titlePic,$albumId,$videoId,$catalog,$summary,$content,$tags,$args);
			return true;
		}
		Log::error($result['errorDescription']);
		return false;
