     ...
     if(!empty($countl)){
        // $data=array();
         for ($j=0; $j<$countl; $j++) {
             ...
             $gid_arr[] = $search['game_idl'.$j];
         }
         
         
     }
     if (count($gid_arr) <> count(array_unique($gid_arr))) {
         return $this->back("添加的游戏不可重复！");
     }
...
//现金流水
    ...
    public function getMoneyRecordsdk()
    {
        $data = $search = $input = array();
        $pageSize = 10;
        $total = 0;
        ...
        $search['isPromotion'] = 'TRUE';
        $search['actionId']="DUOYOU";
        $search['fromActionId']="SDK_ANDROID";
        $res = TuiguangService::excute($search,"money");
        ...
        $data['feeAmount']=$res['data']['feeAmount'];
        $data['count']=isset($res['count'])?$res['count']:'';
        return $this->display('money-sdk-list',$data);
    }
//游币流水
    ...
    public function getCommissionSetup()
    {
        ...
        $res_acc = TuiguangService::excute(array('confKey'=>'duoyouCashLimit'),"account/configlist");
    ...
    public function getCommissionSetupduoyou()
    {
        $data['data'] = Input::get();
        $data['sdk'] =array();

        $res =  TuiguangService::excute(array('actionId'=>'DUOYOU'),"default/list");
//         print_r($res);die();
        if($res['success']&&$res['data']){
            foreach($res['data'] as $k=>$v){
                if($v['actionId']=="DUOYOU"){
                    if($v['feeType']=="0"){
                        $data['sdk']['feeType'] = "0";
                        $data['sdk']['rmbReward'] = $v['feeValueShow'];
                        $data['sdk']['rmbRate'] = "";
        ...
        return $this->display('commission-setupduoyou',$data);
    }
    
    public function getCommissionSetupsdk()
    {
        $data['data'] = Input::get();
        $data['sdk'] =array();
        
        $res =  TuiguangService::excute(array('actionId'=>'SDK_ANDROID'),"default/list");
        //         print_r($res);die();
        if($res['success']&&$res['data']){
            foreach($res['data'] as $k=>$v){
                if($v['actionId']=="SDK_ANDROID"){
                    if($v['feeType']=="0"){
                        $data['sdk']['feeType'] = "0";
                        $data['sdk']['rmbReward'] = $v['feeValueShow'];
                        $data['sdk']['rmbRate'] = "";
        ...
        return $this->display('commission-setupsdk',$data);
    }
    
    public function getCommissionSetuplimit()
    {
        $data['data'] = Input::get();
        $data['cash'] =array();
        $res_acc = TuiguangService::excute(array('confKey'=>'duoyouCashLimit'),"account/configlist");
        ...
        return $this->display('commission-setuplimit',$data);
    }
 ...
 public function postTransactionUpdata(){
  ...
  public function getGameSearch(){
    ...
    //游戏分成设置
    public function getCommissionGamelist()
    {
        $data = $search = $input =  array();
        $pageSize = 10;
        $total = 0;
...
//        $search['gameRefId'] = Input::get('gameRefId','');
        ...
        if($res['success']){
            ...
            foreach($data['datalist']  as &$v){
                $isStepArr = $noStepArr = array();
                    $v['zc'] = "";

                    foreach($v['gamePercentageBeanList'] as $v1){
                        if($v1['isActive']){
                            if($v1['isStep']){
                                $isStepArr[] = $v1;
                            }else{
...
//                    print_r($isStepArr);print_r($noStepArr);
                    if($isStepArr){
                        foreach($isStepArr as $step){
                            if (!isset($step['amountBeginShow'])) $step['amountBeginShow']=0; 
                            if (!isset($step['amountEndShow'])) $step['amountEndShow']=0;
                            $str = (int)$step['amountBeginShow']."-".(int)$step['amountEndShow']."  ";
                            if($step['feeType'] == "0"){
                                $str .= $step['feeValueShow'];
                            }else{
                                $str .= $step['feeValueShow']."%";
                            }
                            $v['zc'].=$str."<br>";
                        ...
                        if($noStepArr['feeType'] == "0"){
                            $v['zc'] = $noStepArr['feeValueShow'];
                        }else{
                            $v['zc'] = $noStepArr['feeValueShow']."%";
                        }
                    }
//                //需要查找直充分成数据并结合充值分成
//                if($v['gameRefId']){
//                    $res1 = TuiguangService::v4excute(array(
//                        'gameRefId' => $v['gameRefId'],
//                        'gameId' => $v['gameId'],
...
//            print_r($data['datalist']);die;
        ...
        return $this->display('commission-gamelist',$data);
    }
    
    public function getCommissionGamelistsdk()
    {
        $data = $search = $input =  array();
        $pageSize = 10;
        $total = 0;
        ...
        return $this->display('commission-gamelistsdk',$data);
    }

    public function getGameCommission(){
        $data = $search = $input = $isStepArr = $noStepArr = array();
        $input = Input::get();
        //充值分成DUOYOU
        $input['actionId'] = 'DUOYOU';
        $res = TuiguangService::v4excute($input,"queryGame");
        $data['tfcount'] = 0;
        if($res['success']){
            $list = $res['data'][0]['gamePercentageBeanList'];
            foreach($list as $k=>$v){
                if($v['isActive']){
                    if($v['feeType']=="0"){
                        $v['count'] = $v['feeValueShow'];
                    }else{
                        $v['prop'] = $v['feeValueShow'];
                    }
                    if($v['isStep']){
                        $isStepArr[] = $v;
                        $data['tfcount'] = $data['tfcount'] + 1;
                    }else{
                        $noStepArr = $v;
                    }
                }
            }
            $data['isStepArr'] = $isStepArr;
            $data['noStepArr'] = $noStepArr;
            if($isStepArr){
                $data['action'] = "isStep";
                $data['upperLimit'] = $isStepArr[0]['upperLimit']/100;
            }elseif($noStepArr){
                $data['action'] = "noStep";
                $data['upperLimit'] = $noStepArr['upperLimit']/100;
            }
        }
        $data['data'] = $input;

        return $this->display('game-commission',$data);
    }
    public static function postGameCommission()
    {
        $data = Input::get();
        $url = $data['url'];unset($data['url']);
        $search = array(
            'gameRefId' => $data['gameRefId'],
            'gameId' => $data['gameId'],
        );
        $res = TuiguangService::v4excute($search,"queryGame");
        if($res['success']){
            ...
            $list = str_replace('"isActive":true','"isActive":false',$list);
            $list = json_decode($list,true);
            $res['data'][0]['gamePercentageBeanList'] = $list;
            $res = TuiguangService::v4excute($res['data'][0],$url,false);
        }


        $data['gamePercentageBeanList'] = json_decode($data['gamePercentageBeanList'],true);
        $res = TuiguangService::v4excute($data,$url,false);
        echo json_encode($res);
    }
    
    public function getGameCommissionsdk(){
        $data = $search = $input = $isStepArr = $noStepArr = array();
        $input = Input::get();
        //SDK_ANDROID
        $input['actionId'] = 'SDK_ANDROID';
        $res = TuiguangService::v4excute($input,"queryGame");
        if($res['success']){
            $list = $res['data'][0]['gamePercentageBeanList'];
            foreach($list as $k=>$v){
        ...
        return $this->display('game-commissionsdk',$data);
    }
    public static function postGameCommissionsdk()
    {
        $data = Input::get();
        $url = $data['url'];unset($data['url']);
        $search = array(
