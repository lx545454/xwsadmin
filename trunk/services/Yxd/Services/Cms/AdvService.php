...
use Yxd\Services\Models\Adv;
use Yxd\Services\Models\AdvPos;
use Yxd\Services\Models\AppAdv;
use Yxd\Services\Models\AppAdvActiveStat;
use Yxd\Services\Models\AppAdvStat;
use Yxd\Services\Models\GameRecommend;
use Yxd\Services\Models\ArticleRecommendGame;
...
{	
		...
		if($game_id){
			...
			switch($detail_type){
				...
				case self::DETAIL_ADV_NEWGAME:
					$data = ArticleRecommendGame::db()->where('type','=',$detail_type)->where('linkid','=',$linkid)->first();
					break;
	...
	public static function getGuessInfo($appname,$version)
	{
		...
		if(CLOSE_CACHE===false && CacheService::has($cachekey_pos)){
			...
			$guess = GameRecommend::db()->where('type','=','gl')->orderBy('addtime','desc')->first();
			if($guess){
		...
		$count = AccountCircle::db()->where('game_id','=',$adv['gid'])->count();
		$out['viewcount'] = $count;
	...
	public static function stat($appname,$version,$location,$osversion,$advid,$code,$idfa,$openudid,$type,$linkid)
	{
		...
		$tb = AppAdvStat::db()->where('appname','=',$appname)->where('version','=',$version)->where('location','=',$location)->where('addtime','=',$dateline);
		if($advid){
		...
		$adv = $tb->first();
		if($adv){
			AppAdvStat::db()->where('id','=',$adv['id'])->increment('number');
			return true;
		}else{		
			$data = array();
    	   ...
    	   AppAdvStat::db()->insertGetId($data);
    	    return true;
		}
	}
	...
	public static function active($code,$idfa,$advid)
	{
		...
		if($idfa){
			$count = AppAdvActiveStat::db()->where('idfa','=',$idfa)->where('aid','=',$advid)->count(); 
			if($count==0){
				$data['aid'] = $advid;
				$data['code'] = $code;
				$data['idfa'] = $idfa;
				$data['addtime'] = $date;
				AppAdvActiveStat::db()->insertGetId($data);
				return true;
			}
			return null;
		}elseif($code){
			$count = AppAdvActiveStat::db()->where('code','=',$code)->where('aid','=',$advid)->count(); 
			if($count==0){
				$data['aid'] = $advid;
				$data['code'] = $code;
				$data['idfa'] = $idfa;
