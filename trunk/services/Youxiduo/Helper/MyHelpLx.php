...
{		
    ...
    //在结果中插入用户信息，结果中要带用户的id暂时支持accountId、userId、luckyUserId、uid三种key
    public static function insertUserhtmlIntoRes($res){
        foreach($res as &$row){
            ...
            if($user&&isset($user['uid'])){
                $html = '<span class="badge badge-info">'.$user['uid'].'</span><br/>'.$user['nickname'].'<br>'.$user['mobile'].'</a>';
                $row['userhtml'] = $html;
            }
        }
        return $res;
    ...
    public static function longExport($url,$time='10',$title="导出内容"){
        $data = array(
            'excelName' => $title,
            'apiKey' => $url
        );
        ...
        $res = AllService::excute('export',$data,'ExportExcel/GetDataCreateExcel');
        if($res['success']){
            $exportId = $res['data']['exportId'] ? $res['data']['exportId']:false;
        }else{
            $exportId = false;
        }
        if($exportId){
...
//            usleep(500000);//0.5秒
                ...
                $res = AllService::excute('export',$data,'ExportExcel/ExportStatus');
                if($res['success']){
                    if(isset($res['data']['finishTime']) && $res['data']['finishTime']>0){
                        $ret['success'] = true;
                        $ret['data'] = Config::get('app.export_api_url').'ExportExcel/ExprotDownload?exportId='.$exportId;
