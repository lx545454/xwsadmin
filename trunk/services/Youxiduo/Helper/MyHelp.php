...
{		
    ...
    public static function save_img($img,$dir_='adv'){
        $titlePic ="";
        if($img) {
            if (!isset($dir)) {
    ...
    public static function save_file_no_url($url,$dir_='adv'){
        $titlePic = array();
        if($url) {
            if (!isset($dir)) {
                $dir = '/userdirs/'.$dir_.'/' . date('Y') . '/' . date('m') . '/';
            ...
            if($mime == "apk"){
               //if($mimeTye == "application/jar" || $mimeTye == "application/java-archive" || $mimeTye == "application/vnd.android.package-archive"){
                   $file->move($path, $file_name);
                   $titlePic['name'] = $file_name;
                   $titlePic['path'] = $path;
                //}
            }
        }
    ...
    public static function getSendStreamFile($url, $file){
        if(file_exists($file)){
            $opts = array(
                'http' => array(
                    'method' => 'GET',
    ...
    public static function save_imgs($imgs,$dir_='adv'){
        $pic_arr = array();
        if($imgs){
            foreach($imgs as $k=>$v){
                $pic_arr[] = self::save_img($v,$dir_);
            }
        }
        return $pic_arr;
    }
